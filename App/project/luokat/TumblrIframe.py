...
class TumblrIframe(Sarjis):
	...
	def Kuvat(self):
		...
		frame = self.soup.find("iframe", { "class": "photoset" })
		if frame:
			print(frame["src"])
			r = requests.get(frame["src"], headers=app.config["REQUEST_HEADER"] )
			soup = BeautifulSoup(r.text, 'html.parser')
			print(soup)
			
			p = re.findall("http:=\"([^;]*)png", r.text)
